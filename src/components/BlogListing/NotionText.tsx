...
export const NotionText: React.FC<{ text: TextProps[] | null }> = ({ text }) => {
  if (text == null) {
    return null
  }
      ...
      {text.map((value, idx) => {
        ...
        return (
          <Fragment key={idx}>
            {text.link != null ? (
              <>
                <Link href={text.link.url} className="underline hover:text-pink-600">
                  <RenderTextContent isCode={code} content={text.content} className={classes} />
                </Link>
              </>
            ) : (
              <RenderTextContent isCode={code} content={text.content} className={classes} />
